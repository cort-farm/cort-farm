...
const dialogTitles: Record<string, string> = {
  sale: "إضافة عملية بيع", debt: "إضافة دين علي", debtor: "منح قرض",
...
const txTypeMeta: Record<string, { color: string; bg: string; positive: boolean }> = {
  ...
  loan_given: { color: "text-sky-600 dark:text-sky-400", bg: "bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400", positive: false },
...
const txTypeLabel: Record<string, string> = {
  ...
  debtor_added: "مدين جديد", debtor_collected: "تحصيل", withdrawal: "سحب", loan_given: "قرض",
};
...
export default function Dashboard() {
  ...
  const handleGiveLoan = async (data: Parameters<typeof svc.giveLoan>[1]) => {
    await svc.giveLoan(uid, data);
    toast.success("تم تسجيل القرض بنجاح");
  };
    ...
    <div className="space-y-7 pb-24" dir="rtl">
      ...
      <Dialog open={dialog !== null} onOpenChange={close}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl" dir="rtl">
          ...
          {dialog === "debtor" && <AddDebtorForm onSubmit={handleGiveLoan} onClose={close} />}
