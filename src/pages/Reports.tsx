...
export default function Reports() {
  const { sales, transactions, debtors, withdrawals, sellingCostsTotal } = useFinanceData();
  ...
  const monthlyData = useMemo(() => {
    return last6Months.map(({ year, month, label }) => {
      const monthSales = sales.filter(s => { const d = new Date(s.date); return d.getFullYear() === year && d.getMonth() === month; });
      const totalSales = monthSales.reduce((sum, s) => sum + s.totalSale, 0);
      const totalProfit = monthSales.reduce((sum, s) => sum + s.netProfit, 0);
      const collections = transactions
        .filter(t => t.type === "debtor_collected" && new Date(t.date).getFullYear() === year && new Date(t.date).getMonth() === month)
        .reduce((sum, t) => sum + t.amount, 0);
      const withdrawalSum = withdrawals
        .filter(w => new Date(w.date).getFullYear() === year && new Date(w.date).getMonth() === month)
        .reduce((sum, w) => sum + w.amount, 0);
      return { label, totalSales, totalProfit, collections, withdrawals: withdrawalSum };
    });
  }, [sales, transactions, withdrawals, last6Months]);

  const topDebtors = useMemo(() => {
    return [...debtors].filter(d => d.remaining > 0).sort((a, b) => b.remaining - a.remaining).slice(0, 5).map(d => ({ name: d.name, value: d.remaining }));
  }, [debtors]);

  const currentMonth = now.getMonth();
  const currentYear = now.getFullYear();
  const thisMonthSales = sales.filter(s => { const d = new Date(s.date); return d.getMonth() === currentMonth && d.getFullYear() === currentYear; });
  const monthlyProfit = thisMonthSales.reduce((sum, s) => sum + s.netProfit, 0);
  const monthlyTotal = thisMonthSales.reduce((sum, s) => sum + s.totalSale, 0);
  const totalCollections = transactions.filter(t => t.type === "debtor_collected").reduce((sum, t) => sum + t.amount, 0);
  ...
  const pieData = [
    ...
    { name: "تكاليف", value: sellingCostsTotal },
    { name: "سحوبات", value: totalWithdrawals },
  ].filter(d => d.value > 0);
  ...
  const summaryCards = [
    ...
    { label: "إجمالي السحوبات", value: totalWithdrawals, icon: Wallet, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-100 dark:bg-amber-900/30", border: "border-amber-200/60 dark:border-amber-800/40" },
  ];
