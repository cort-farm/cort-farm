...
export default function AddSaleForm({ onSubmit, onClose }: Props) {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      productName: "", units: 1, salePrice: 0, sellingCost: 0,
      costDescription: "", date: getTodayString(), paymentMethod: "cash",
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormField control={form.control} name="productName" render={({ field }) => (
            <FormItem className="col-span-2">
              ...
              <FormControl><Input {...field} placeholder="اسم المنتج" /></FormControl>
              <FormMessage />
            </FormItem>
          )} />
          ...
          <FormField control={form.control} name="costDescription" render={({ field }) => (
            <FormItem>
              ...
              <FormControl><Input {...field} placeholder="مثال: شحن" /></FormControl>
              <FormMessage />
            </FormItem>
          )} />
        </div>
          ...
          <div className="grid grid-cols-2 gap-3 border border-border rounded-xl p-3">
            ...
            <FormField control={form.control} name="debtorName" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input {...field} placeholder="الاسم" /></FormControl>
              </FormItem>
            )} />
            <FormField control={form.control} name="debtorPhone" render={({ field }) => (
              <FormItem>
                <FormLabel>رقم الهاتف</FormLabel>
                <FormControl><Input {...field} placeholder="05xxxxxxxx" /></FormControl>
              </FormItem>
            )} />
          </div>
        )}
