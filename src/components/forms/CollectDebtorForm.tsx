...
import type { Debtor } from "@/stores/useFinanceStore";
...
export default function CollectDebtorForm({ debtors, onSubmit, onClose }: Props) {
  ...
  const handleSubmit = async (data: FormData) => {
    setLoading(true);
    try { await onSubmit(data.debtorId, data.amount, data.date); onClose(); } finally { setLoading(false); }
  };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        ...
        <div className="flex gap-2 pt-2">
          <Button type="submit" className="flex-1" disabled={loading || activeDebtors.length === 0}>
