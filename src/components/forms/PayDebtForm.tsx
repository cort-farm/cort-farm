...
import type { Debt } from "@/stores/useFinanceStore";
...
export default function PayDebtForm({ debts, onSubmit, onClose }: Props) {
  ...
  const handleSubmit = async (data: FormData) => {
    setLoading(true);
    try { await onSubmit(data.debtId, data.amount, data.date); onClose(); } finally { setLoading(false); }
  };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormField control={form.control} name="amount" render={({ field }) => (
            <FormItem>
              ...
              <FormControl><Input {...field} type="number" min="0" step="0.01" max={remaining || undefined} /></FormControl>
              <FormMessage />
            </FormItem>
          )} />
        ...
        <div className="flex gap-2 pt-2">
          <Button type="submit" className="flex-1" disabled={loading || activeDebts.length === 0}>
